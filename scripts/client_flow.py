#!/usr/bin/env python3
"""
TCoder Client Flow Script - Fully Automatic

Complete automated workflow:
1. Request presigned upload URL
2. Upload video file to R2
3. Manually trigger R2 event processing (simulates webhook notification)
4. Poll for job completion

Usage:
    python client_flow.py video.mp4
    python client_flow.py video.mp4 --base-url http://localhost:8787
"""
...
class TcoderClient:
    ...
    def _request(self, method: str, url: str, data: dict = None, headers: dict = None) -> dict:
    ...
    def request_upload_url(
        ...
        preset: str = "default",
        output_qualities: list[str] = None,
    ) -> dict:
    ...
    def trigger_r2_event(self, job_id: str) -> dict:
        """Manually trigger R2 event processing (simulates webhook notification)"""
        url = f"{self.api_base}/jobs/{job_id}/trigger-r2-event"
    ...
    def _print_job_results(self, status: dict) -> None:
        ...
        if timestamps:
            ...
            if "completedAt" in timestamps:
                ...
                # Calculate duration
                if "startedAt" in timestamps:
                    duration = (timestamps["completedAt"] - timestamps["startedAt"]) / 1000
...
def main():
    parser = argparse.ArgumentParser(
        description="TCoder Client Flow - Fully automatic transcoding workflow",
    ...
    parser.add_argument(
        ...
        help="Base URL of the API server (default: http://localhost:8787)",
    )
    ...
    client = TcoderClient(base_url=args.base_url)

    try:
        # Step 1: Request upload URL
        upload_data = client.request_upload_url(
        ...
        # Step 3: Manually trigger R2 event processing (simulates webhook notification)
        client.trigger_r2_event(job_id)
